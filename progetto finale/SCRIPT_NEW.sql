...
DROP temporary table if exists num_trans;
...
drop temporary table if exists tot_trans;
...
drop temporary table if exists num_tot_conti;
...
drop temporary table if exists num_tot_conti_tipologia;
create temporary table num_tot_conti_tipologia as (
	select 
		clien.id_cliente as id_cliente,
...
drop temporary table if exists num_trans_tipologia;
create temporary table num_trans_tipologia as (
	select
	 clien.id_cliente as id_cliente,
		CASE
		when tipo_t.segno = '+' then 'accredito'
...
drop temporary table if exists tot_trans_tipologia;
create temporary table tot_trans_tipologia as (
select
	clien.id_cliente as id_cliente,
	CASE
		when tipo_t.segno = '+' then 'accredito'
